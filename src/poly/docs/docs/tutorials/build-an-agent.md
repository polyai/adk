...
This guide walks through how to go from a blank slate to a production-ready voice agent with a real backend using **Agent Studio**, the **PolyAI ADK**, and a coding agent such as **Claude Code**.
...
-   **The coding agent builds**
    ...
    Using the ADK, the coding agent generates the project files needed for the agent.
...
!!! info "No manual flow-building required"
    ...
    This workflow is designed so that the coding agent does the heavy lifting of building the agent, while Agent Studio remains the place where the work is reviewed, tested, and deployed.
...
| **Coding agent + ADK** | Generate project files and push changes |
