...
async def run_call(session: CallSession, caller: str, *, aec: bool = False) -> None:
    ...
    try:
        async with websockets.connect(signaling_url(session.gateway_ws_url)) as ws:
            offer = build_offer_message(session, pc.localDescription.sdp, caller)
            await ws.send(json.dumps(offer))
            await _run_until_done(pc, ws, connection_failed)
    finally:
        microphone.stop()
        speaker.stop()
        await pc.close()
...
async def _run_until_done(pc: RTCPeerConnection, ws, connection_failed: asyncio.Event) -> None:
    ...
    done, pending = await asyncio.wait({signaling, failure}, return_when=asyncio.FIRST_COMPLETED)
