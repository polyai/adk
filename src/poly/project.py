...
class AgentStudioProject:
    ...
    def pull_project(
        ...
        files_with_conflicts = self._apply_pulled_resources(
            incoming_resources, force=force, format=format, on_save=on_save
        )
    ...
    def pull_project_from_env(self, env: str = "sandbox", format: bool = False) -> list[str]:
        ...
        return self._apply_pulled_resources(
            incoming_resources, force=True, format=format, on_save=None
        )

    def _apply_pulled_resources(
        self,
        ...
        """Apply pulled resources to the local project and return files with conflicts.

        Merges the incoming resources into the local files (overwriting on force),
        cleans up empty flow folders, updates the in-memory project state, deletes
        local-only resources when forcing, and regenerates decorators/imports.

        Args:
            incoming_resources (ResourceMap): Resources pulled from the remote.
            force (bool): If True, overwrite local changes and delete local-only
                resources.
            format (bool): If True, format the resource before saving.
            on_save (Callable[[int, int], None] | None): Optional progress callback.
