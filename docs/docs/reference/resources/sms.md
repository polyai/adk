...
!!! note "Enterprise feature"
    ...
    SMS templates require an enterprise Agent Studio account.
...
| `text` | Message body. Supports [`{{vrbl:variable_name}}`](./variables.md) placeholders from `conv.state` and [`{{tn:translation_name}}`](./translations.md) references. No other reference type (e.g. `{{fn:...}}`, `{{ho:...}}`) is valid here. |
...
## Using variables and translations
...
It can also reference a [translation](./translations.md) key, for example:

~~~text
{{tn:booking_confirmed}}
~~~
...
- Only `{{vrbl:...}}` and `{{tn:...}}` references are valid inside `text` — using any other reference type fails validation.
...
-   **Translations**
    ...
    Reference a localized string from template text with `{{tn:...}}`.
