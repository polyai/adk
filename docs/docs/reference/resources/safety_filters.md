...
Each category has its own `enabled` flag and `level` — see [Validation](#validation) for which combinations are required.
...
- `chat/safety_filters.yaml` can only be pushed together with the other webchat config files ([chat settings](./chat_settings.md) greeting and style prompt) — `poly push` rejects it if a sibling webchat file is missing.
