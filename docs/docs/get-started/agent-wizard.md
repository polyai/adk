---
title: Not sure where to start? Try Agent Wizard
description: Learn how to use Agent Wizard to build your first PolyAI agent in minutes, then connect it to the ADK for local development.
---

# Not sure where to start? Try Agent Wizard

If you do not yet have an agent in Agent Studio, or if you are feeling stuck before diving into local development, **Agent Wizard** is the fastest way to get a working voice agent off the ground.

Agent Wizard builds a personalised agent from your company website in a few minutes — no configuration required. The agent it creates lives in Agent Studio as a normal project, which means you can pull it straight into the ADK and continue development locally the moment it is ready.
...
!!! tip "What the wizard generates"

    The wizard creates flows, topics, and agent settings based on your website's public content. Everything it produces is standard ADK-compatible configuration — you can pull it down and edit every file locally.

### Step 3 — Test your agent in Agent Studio

Once the wizard finishes, test the agent inside Agent Studio to confirm it responds as expected. This gives you a working baseline before you move to local development.

### Step 4 — Find your account and project IDs

To pull the wizard-created agent into the ADK, you need two identifiers from Agent Studio. You can find them in the URL when your project is open:
...
Once the [ADK is installed](./installation.md), link your local folder to the wizard-created project and pull its configuration down:
...
If you already have an agent in Agent Studio — built via Agent Wizard, in the browser editor, or by a PolyAI team — skip the wizard. The ADK connects to any existing Agent Studio project using the same `poly init` + `poly pull` workflow described above.
...
Once you have an agent in Agent Studio — whether from the wizard or already there — continue to the prerequisites page to set up your API key and local tools.
